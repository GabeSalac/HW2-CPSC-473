...
var PREV_LINK_SELECTOR = '[data-image-role="prev"]'
...
function addNextClickHandler() {
  ...
  var button = document.querySelectorAll(NEXT_LINK_SELECTOR);
  var buttonArr = [].slice.call(button);
  var nextButton = buttonArr[0];
  ...
  nextButton.addEventListener("click", function(event) {
    ...
    for (var i = 0; i < thumbnailsArr.length; i++) {
      if (thumbnailsArr[i].getAttribute("data-image-title") == getTitle.textContent) {
...
function addPrevClickHandler() {
  'use strict';
  var getTitle = document.querySelector(DETAIL_TITLE_SELECTOR);
  var button = document.querySelectorAll(PREV_LINK_SELECTOR);
  var buttonArr = [].slice.call(button);
  var prevButton = buttonArr[0];
  var thumbnailsArr = getThumbnailsArray();
  var currImage;
  var currTitle;
